...
exports.addexpense = async (req, res, next) => {
    ...
    try {
        ...
        const currentUser = await userdb.findOne( { _id: req.user.id });
        ...
        await userdb.findByIdAndUpdate(req.user.id,{totalExpense: updatedTotalExpense}
...
exports.getexpense = async (req, res, next) => {
    try {
        ...
        const totalexpense = await alldetails.countDocuments({ userId: req.user.id });
        ...
        const expenses = await alldetails.find({
            userId: req.user.id,
        })
...
exports.deleteexpense = async (req, res) => {
    ...
    try {
        ...
        const expenseToDelete = await alldetails.findOne( { _id: detailsId, userId: req.user.id });

        // Capture the expens value of the expense to be deleted
        const expens = expenseToDelete.expens;

        const data = await alldetails.findByIdAndDelete( { _id: detailsId, userId: req.user.id });
...
exports.downloadexpense = async (req, res) => {
    try {
        ...
        const fetchedData = await downloaddb.find( { userId: req.user.id } );
...
exports.paginateExpenses=async(req,res)=>{
    try{
      ...
      const data=  await expensedatabase.find({
        offset:(page)*10,
        limit:10,
