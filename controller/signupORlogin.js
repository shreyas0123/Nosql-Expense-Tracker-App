...
exports.signup = async (req,res,next) =>{
    try{
        ...
        const uniqueEmail = await signUpUserDetails.findOne({email:email});
...
exports.login = async (req,res) =>{
    try{
        ...
        const uniqEmail = await signUpUserDetails.findOne({email:email})
        ...
        if(uniqEmail){
            ...
            const passwordMatch = await bcrypt.compare(password,uniqEmail.password);
              if(passwordMatch){
