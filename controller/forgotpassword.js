...
exports.resetpassword = async (req, res) => {
    try {
        ...
        if (forgotPassword) {
            ...
            await forgotPassword.updateOne({ isactive: false });
...
exports.updatepassword = async (req, res) => {
    try {
        ...
        // Find the corresponding reset password entry in the 'forgotpassword' table
        const findUser = await forgotpassword.findOne({ id: resetpassword } );

        // Find the user associated with the 'userId' from the 'forgotpassword' entry
        const data = await user.findOne( { id: findUser.userId } );
        ...
        if (data) {
            ...
            bcrypt.hash(newpassword, 10, async (err, hash) => {
                ...
                const data2 = await data.updateOne({ password: hash });
