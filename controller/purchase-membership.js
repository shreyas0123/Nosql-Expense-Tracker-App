...
exports.getPremium = async (req, res, next) => {
    try {
        ...
        Razor.orders.create({ amount, currency: "INR" }, async (err, order) => {
            ...
            // Create the order in the database, including the paymentId
            await orderDb.create({
                orderId: order.id,
                status: "PENDING",
                userId: req.user.id,
                paymentId: null  // Set to null initially, as the payment has not been made yet
            });
...
function generateToken(_id,ispremiumUser){
    return jwt.sign({userId: _id,isPremium: ispremiumUser},'fiuhf2bd484fdfhfff656ffhfEwddfkmnv');
}
...
exports.updatePremium = async (req, res, next) => {
    try {
        ...
        if (!paymentid) {
            ...
            return result.updateOne({ paymentId: paymentid, status: "FAILED" });
        }
        ...
        function updateTable(result) {
            return new Promise((resolve) => {
                resolve(result.updateOne({ paymentId: paymentid, status: "SUCCESS" }));
            });
        }
        ...
        function updateUserTable() {
            return new Promise((resolve) => {
                resolve(req.user.updateOne({ ispremiumUser: true }));
            });
        }
